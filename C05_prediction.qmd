...
Finally we come to the end product of forecasting: the prediction. This last step is actually fairly simple, given a recipe and model (now bundled in a `workflow` container).  We have already made predictions using tables of input covariates, but we now want to predict across the entire domain of our Brickman data set.  This requires predicting with `stars` objects not tables. You may recall that we are able to read these arrays, display them and extract point data from them. But we haven't used them *en mass* as a variable yet.
...
```
:::{.callout-note}
Curious about why we provide year as a vector of characters instead of a vector of integers?  Try running the command above again but this time using `along = list(year = c(2020, 2055, 2075))` (as numbers not strings). Then check out the 3rd dimension.
:::
